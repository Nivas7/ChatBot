...
import { DEFAULT_OPENAI_MODEL } from "@/Shared/constant";
...
export default async function handler(
  ...
  const body = req.body;
  const messages = (body?.messages || []) as CreateChatCompletionRequestMessage[];
  ...
  try {
    ...
    const initialMessages: CreateChatCompletionRequestMessage[] = messages.splice(
      0,
      3
    );
    const latestMessages: CreateChatCompletionRequestMessage[] = messages
      .slice(-5)
      .map((message) => ({
        role: message.role,
